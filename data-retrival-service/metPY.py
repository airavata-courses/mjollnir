...
class metPY:
    ...
    def imageWriter(self,radar_id, year, month, date, hours):
        ...
        for scan in availscans:
            ...
            try:
                print("hello")
                f = Level2File('./'+queryString);print(f)
                sweep = 0
                ...
                add_metpy_logo(fig, 190, 85, size='large');print("here 1")
                ...
                plt.switch_backend('Agg');print("here 2")
                plt.savefig(queryString, format='png')
                path = os.getcwd() + '/' + queryString;print("here 3")
                with open(path, 'rb') as output:
                    encoded = base64.b64encode(output.read())
                    json = {'image': encoded.decode()}
                    output.close()
                dirs = os.listdir();print("here 4")
                cur_dir = os.getcwd()
                for file in dirs:
